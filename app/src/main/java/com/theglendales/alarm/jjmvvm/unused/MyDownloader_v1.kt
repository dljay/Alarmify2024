...
class MyDownloader_v1(private val receivedActivity: Activity) : AppCompatActivity() {
    ...
    private val READ_PERMISSION_CODE: Int =
        2480 // Read. 위에 download 가 되면(당연히 write 가 되므로) read permission 은 그냥 pass.
    ...
    fun multiDownloadOrNot(downloadableItem: DownloadableItem, keepTheFile: Boolean) {
        ...
        Log.d(
            TAG,
            "MultiDownloadOrNot: listToBeJudged.size=${listToBeJudged.size}, itemTobeJudged[TrkId]=${downloadableItem.trackID}"
        )
        ...
        if (!keepTheFile) {
            Log.d(
                TAG,
                "MultiDownloadOrNot: XX trackId=${downloadableItem.trackID} (는)  구매한적이 없거나(어떤 연유로 불량 구매). 디스크에 있으면 삭제.."
            )
        ...
        //todo: 만약 이 size 가 일치하지 않는 경우 반드시 대비 필요!!할까..?
        {
            ...
            for (i in 0 until listToBeJudged.size) // until 은 마지막 숫자 제외!! => i in 0 until 3 => 0,1,2 까지.
            {
                ...
                if (checkDuplicatedFileOnDisk(filePath)) { // 파일이 이미 디스크에 있는 상태
                    ...
                    Log.d(
                        TAG,
                        "MultiDownloadOrNot: [i]=$i, listToBeJudged.size=${listToBeJudged.size} // OO: $fileName 가(이) 이미 디스크에 있어서 다운 필요 없음. 경로: '$filePath"
                    )
                    ...
                    Log.d(
                        TAG,
                        "MultiDownloadOrNot: @@ finalList.size=${finalList.size} // @@ XX: $fileName (은) 디스크에 없어서 다음 파일을 다운받아야함=$filePath"
                    )
                }
            }
            ...
            //permHandler.permissionForMultipleDNLD(finalList) //
    ...
    fun singleDownloadOrNot(downloadableItem: DownloadableItem, keepTheFile: Boolean) {
        ...
        Log.d(
            TAG,
            "SingleDownloadOrNot: listToBeJudged.size=${listToBeJudged.size}, itemTobeJudged[TrkId]=${downloadableItem.trackID}"
        )

        if (!keepTheFile) {
            Log.d(
                TAG,
                "SingleDownloadOrNot: XX trackId=${downloadableItem.trackID} (는)  구매한적이 없거나(어떤 연유로 불량 구매). 디스크에 있으면 삭제.."
            )
            deleteFromDisk(downloadableItem)
            // todo : add return?
        }
        ...
        if (checkDuplicatedFileOnDisk(filePath)) { // 파일이 이미 디스크에 있는 상태
            Log.d(
                TAG,
                "SingleDownloadOrNot:  $fileName 가(이) 이미 디스크에 있어서 다운 필요 없음 XX. 경로: '$filePath"
            )
        } else if (!checkDuplicatedFileOnDisk(filePath)) {
            finalList.add(downloadableItem)
            Log.d(
                TAG,
                "SingleDownloadOrNot:  $fileName (은) 디스크에 없어서 다음 파일을 다운받아야함 OO filepath=$filePath"
            )
        }
    ...
    fun checkDuplicatedFileOnDisk(filePathAndName: String): Boolean { //다운로드 받기 전 이미 Disk 에 File 이 있는지 체크.
        ...
        return if (fileToCheck.isFile) { //true if and only if the file denoted by this abstract pathname exists and is a normal file; false otherwise
            ...
            Log.d(
                TAG,
                "checkDuplicatedFileOnDisk: \"File ${fileToCheck.name} DOES NOT!!XX exist!!\""
            )
    ...
    //********MULTIPLE File Dnld ---------------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    class DownloadInfoContainer(
        val dnldID: Long = 0,
        val trkId: Int = 0,
        val fileNameAndPath: String = ""
    )
    ...
    fun multipleFileDNLD(needToSyncListReceived: MutableList<DownloadableItem>) {
        ...
        val dnldInfoContainerList =
            ArrayList<DownloadInfoContainer>() // 여기에 downloadId, trkId, fileNamePath 저장.
        ...
        for (i in 0 until needToSyncListReceived.size) {
            ...
            if (!isUrlValid) {
                ...
                Toast.makeText(receivedActivity, "DOWNLOAD ERROR. INVALID URL!", Toast.LENGTH_LONG)
                    .show()
                return
            }
            Log.d(
                TAG,
                "multipleFileDNLD: %%%%%%%%%%% WE'll FINALLY DOWNLOAD THIS trId=$trackID %%%%%%%%%%%"
            )
            Log.d(
                TAG,
                "multipleFileDNLD: %%%%%%%%%%% WE'll FINALLY DOWNLOAD THIS URL= $mp3UrlToDownload %%%%%%%%%%%"
            )
            ...
            dnlRequest.setDestinationInExternalFilesDir(
                receivedActivity.applicationContext,
                ".AlarmRingTones",
                fileNametoSaveOnDisk
            )

            //get download service, and enqueue file
            val dnlManager =
                receivedActivity.getSystemService(Context.DOWNLOAD_SERVICE) as DownloadManager
            val downloadID = dnlManager.enqueue(dnlRequest)
            val infoContainerInstance =
                DownloadInfoContainer(downloadID, trackID, fileNameAndFullPath)
        ...
        CoroutineScope(Dispatchers.IO).launch {
            dnldInfoContainerList.forEach { infoContainerObj ->
                launch {
                    ...
                    val downloadResult =
                        getDnldResultMulti(infoContainerObj) // 여기서 대기!! 다운로드 하나가 끝나면 이쪽으로 결과를 보고.
                    Log.d(
                        TAG,
                        "multipleFileDNLD: Coroutine-forEach. COMPLETED DNLDID=${infoContainerObj.dnldID}"
                    )
                    if (downloadResult) { // true 를 받으면=> 즉 하나가 끝났으면.
                        dnldCounter++
                    }
                    var totalProgress: Float =
                        dnldCounter.toFloat() / dnldInfoContainerList.size.toFloat() * 100F
                    ...
                    runOnUiThread { // 여기서 여러 파일의 다운로드 prgrs animation 진행!!
                        ...
                        if (dnldCounter > 0) { //하나라도 다운로드 완료되면-> Progress animation 시작
                            Log.d(
                                TAG,
                                "multipleFileDNLD: totalProgress=$totalProgress. dnldCounter.tofloat=${dnldCounter.toFloat()}, dnldInfoContList.size=${dnldInfoContainerList.size}"
                            )
                            btmShtMultiObj.animateLPI(
                                totalProgress.toInt(),
                                4000
                            ) // 애니메이션이 차는 속도 4초.
                        }
                        if (dnldCounter == dnldInfoContainerList.size) { // 100 퍼 만땅 animation 위해 그냥 넣음..
                            ...
                            btmShtMultiObj.lpiMulti.progress =
                                0 // 초기화. 이거 안하면 나중에 프로그레스바 anim 그래프 가운데서 시작하고 막 그러네
                        }
                    ...
                    if (dnldCounter == dnldInfoContainerList.size) { // 전체 다운로드 완료!!
                        Log.d(
                            TAG,
                            "multipleFileDNLD: !!ALL DOWNLOADS/SYNC COMPLETED. dnldCounter=$dnldCounter"
                        )
            ...
            if (cursor.moveToFirst()) {
                ...
                when (status) {
                    DownloadManager.STATUS_PENDING -> {
                        Log.d(
                            TAG,
                            "showDNLDProgress: STATUS PENDING  DNLDId=${infoContainer.dnldID}"
                        )
                    }
                    DownloadManager.STATUS_PAUSED -> {
                        ...
                        if (statusPauseCount == 1) {
                            Log.d(
                                TAG,
                                "showDNLDProgress: STATUS PAUSED  <1st!/ trkId=${infoContainer.trkId}"
                            )
                        }
                        if (statusPauseCount == 400) {
                            Log.d(
                                TAG,
                                "showDNLDProgress: STATUS PAUSED  <400th>/ trkId=${infoContainer.trkId}"
                            )
                            statusPauseCount = 0
                        }
                    ...
                    DownloadManager.STATUS_RUNNING -> {
                        ...
                        if (statusRunningCount == 1) {
                            Log.d(
                                TAG,
                                "showDNLDProgress: STATUS RUNNING <1st!>/ trkId=${infoContainer.trkId}"
                            )
                        }
                        if (statusRunningCount == 400) {
                            Log.d(
                                TAG,
                                "showDNLDProgress: STATUS RUNNING <400th>/ trkId=${infoContainer.trkId}"
                            )
                    ...
                    DownloadManager.STATUS_FAILED -> {
                        Log.d(
                            TAG,
                            "showDNLDProgress: STATUS FAILED / DNLDId=${infoContainer.dnldID}"
                        )
                        Toast.makeText(
                            receivedActivity,
                            "Failed to Download. Please Check your Internet connection.",
                            Toast.LENGTH_LONG
                        ).show()
                        btmShtMultiObj.removeMultiDNLDBtmSheet()
                        isSyncInProcess = false
                    ...
                    DownloadManager.STATUS_SUCCESSFUL -> {
                        Log.d(
                            TAG,
                            "showDNLDProgress: STATUS SUCCESSFUL, trkId= ${infoContainer.trkId}, DNLDId=${infoContainer.dnldID}"
                        )
                ...
                val myDownloadProgress =
                    ((bytesDownloaded * 100L) / bytesTotal).toInt() // changed from *100 to *99

                val fileForSizeCheck = File(infoContainer.fileNameAndPath)
                if (fileForSizeCheck.length()
                        .toInt() == bytesTotal
                ) { // STATUS SUCCESSFUL 이 늦게 뜰때는 여기에 의존!
                    // 다운받은 파일크기 == 전체 파일 크기 <- 이건 다운 완료 후 10초내로 응답하는 듯..나름 reliable. BcReceiver 로 전달되는것보다 30초~1분정도 빠름.
                    val currentTime = Calendar.getInstance().time
                    Log.d(
                        TAG,
                        "getResultFromMULTIDNLD: @@@@@@fileSizeCheck.length=${fileForSizeCheck.length()}, name=${fileForSizeCheck.name}, time:$currentTime"
                    )
                    isStillDownloading = false
                }
...
// <----------***MULTIPLE File Dnld <<<<<<<<<<<<----------------
    ...
    fun singleFileDNLD(itemToDownloadable: DownloadableItem) {
        ...
        Log.d(
            TAG,
            "singleFileDNLD: %%%%%%%%%%% WE'll FINALLY DOWNLOAD THIS trId=$trackID %%%%%%%%%%%"
        )
        Log.d(
            TAG,
            "singleFileDNLD: %%%%%%%%%%% WE'll FINALLY DOWNLOAD THIS URL= $mp3UrlToDownload %%%%%%%%%%%"
        )
        val dnlRequest = DownloadManager.Request(Uri.parse(mp3UrlToDownload))
        // 다운로드 방식 두가지 다 .. allowed
        dnlRequest.setAllowedNetworkTypes(DownloadManager.Request.NETWORK_WIFI or DownloadManager.Request.NETWORK_MOBILE)
...
//        dnlRequest.setDestinationInExternalPublicDir("/myTest0413", "${System.currentTimeMillis()}")
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            val isStillDNLDING = getResultFromSingleDNLD(
                downloadID,
                trackID,
                fileNameAndFullPath
            ) // (1) showDNLDCoroutine 시작->
            withContext(Dispatchers.Main) {
                if (!isStillDNLDING) {
                    Log.d(
                        TAG,
                        "^^^^^^^^^^^singleFileDNLD: INSIDE COROUTINE. isStillDNLDING=$isStillDNLDING"
                    )
                    runOnUiThread {
    ...
    private fun getResultFromSingleDNLD(dnldID: Long, trId: Int, filePathAndName: String): Boolean {
        ...
        while (isStillDownloading) {
            ...
            if (cursor.moveToFirst()) {
                ...
                if (fileSizeCheck.length()
                        .toInt() == bytesTotal
                ) { // 다운받은 파일크기 == 전체 파일 크기 <- 이건 다운 완료 후 10초내로 응답하는 듯..나름 reliable..?
                    val currentTime = Calendar.getInstance().time
                    Log.d(
                        TAG,
                        "getResultFromSingleDNLD: @@@@@@fileSizeCheck.length=${fileSizeCheck.length()}, time:$currentTime"
                    )
                    isStillDownloading = false
                }
                ...
                runOnUiThread {
                    ...
                    if (!animInitialBool && status == DownloadManager.STATUS_RUNNING) { //status running = 2
                        ...
                        Log.d(
                            TAG,
                            "getResultFromSingleDNLD: Line454 (animatLPI 부르기 직전! status= STATUS_RUNNING)"
                        )
                        btmShtSingleDNLDInstance.animateLPI(
                            randomPrgrsValue,
                            5000
                        ) // 애니메이션이 차는 속도 5초.
                        ...
                        Log.d(
                            TAG,
                            "getResultFromSingleDNLD: initialAnimation! 'Actual Prgrs'=$myDownloadProgress, randomValue=$randomPrgrsValue, status=$status"
                        )
                    }
                    ...
                    if (!animFortyNSixtyBool && myDownloadProgress > 40 && myDownloadProgress < 60) { // 40~60 구간 animation
                        ...
                        Log.d(
                            TAG,
                            "getResultFromSingleDNLD: animFortyNSixtyBool=$animFortyNSixtyBool, 'Actual Prgrs'=$myDownloadProgress, randomDur=$randomDuration"
                        )
                    }
                    if (!animSixtyEightyBool && myDownloadProgress >= 60 && myDownloadProgress < 80) { // 40~60 구간 animation
                        ...
                        Log.d(
                            TAG,
                            "getResultFromSingleDNLD: animSixtyEightyBool=$animSixtyEightyBool, 'Actual Prgrs'=$myDownloadProgress, randomDur=$randomDuration"
                        )
                    }
                    if (!animEightyOrHigher && myDownloadProgress >= 80) { // 80 구간 animation
                        ...
                        Log.d(
                            TAG,
                            "getResultFromSingleDNLD: animEightyOrHigherBool=$animEightyOrHigher, 'Actual Prgrs'=$myDownloadProgress, randomDur=$randomDuration"
                        )
                    }
    ...
    fun openBtmShtMultiDNLD() { // 다운로드와 동시에 BtmSheet_Sync 호출.
        ...
        if (fm == null) {
            Log.d(
                TAG,
                "openBtmShtMultiDNLD: fm is null==bottomSheet NOT(XX) displayed or prepared!!!!"
            )
        }
        ...
        if (!btmShtMultiObj.isAdded) {//아무것도 display 안된 상태.
            ...
            Log.d(
                TAG,
                "openBtmShtMultiDNLD: ***DISPLAY BOTTOM SHEET_MULTI! NOW!! .isAdded= FALSE!!.."
            )
        }
    }
    ...
    fun openBtmShtSingleDNLD() { // 다운로드와 동시에 BtmSheet_Sync 호출.
        ...
        if (fm == null) {
            Log.d(
                TAG,
                "openBtmShtSingleDNLD: fm is null==bottomSheet NOT(XX) displayed or prepared!!!!"
            )
        }
        if (fm != null) { // BottomSheet 이 display 된 상태.
