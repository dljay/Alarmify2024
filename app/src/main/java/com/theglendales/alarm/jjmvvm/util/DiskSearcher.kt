...
class DiskSearcher(val context: Context)
{
    ...
    fun rtSearcher(): List<Uri>
    {
        ...
        // 만약 폴더가 없을때는 폴더를 생성
        if(!myDir.exists()) {
            Log.d(TAG, "rtSearcher: Folder $myDir doesn't exist. We'll create one")
            myDir.mkdir()
        }
        // 폴더는 있는데 파일이 없을때.. 그냥 return
        if(myDir.listFiles().isNullOrEmpty()) {
            Log.d(TAG, "rtSearcher: NO FILES INSIDE THE FOLDER!")
            return emptyUriList
        }
        if(myDir.listFiles() != null)
        {
            for(f in myDir.listFiles())
            {
                ...
                try { // 미디어 파일이 아니면(즉 Pxx.rta 가 아닌 파일은) setDataSource 하면 crash 남! 따라서 try/catch 로 확인함.
                    ...
                    Log.d(TAG, "rtSearcher: unable to run mmr.setDataSource for the file=${f.name}. WE'LL DELETE THIS PIECE OF SHIT!")
                    f.delete()
                    return emptyUriList
                }
                ...
                Log.d(TAG, "rtSearcher: fileName= ${f.name}, fileDuration=$fileDuration")
                if(fileDuration==null) {
                    Log.d(TAG, "rtSearcher: Possible Corrupted file. Filename=${f.name}")
                    f.delete()
                }
                ...
                if(f.name.contains('-')||!f.name.contains('p')||f.length()==0L) {
                    ...
                    if(f.length()==0L) {
                        Log.d(TAG, "rtSearcher: filesize prob 0? Filesize=${f.length()}")
                    }
                    f.delete()
