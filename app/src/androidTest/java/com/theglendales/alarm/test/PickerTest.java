...
import com.theglendales.alarm.configuration.ContainerKt;
...
public class PickerTest extends BaseTest {
    public ActivityScenarioRule<AlarmsListActivity> listActivity = new ActivityScenarioRule<AlarmsListActivity>(AlarmsListActivity.class);
...
//        onView(withId(R.id.fab)).perform(click());
    ...
    private void assertState(Next next, QuickMinutes quickMinutes, Completion completion) {
        switch (next) {
            case ANY:
                for (ViewInteraction view : CollectionsKt.arrayListOf(zero(), one(), two(), three(), four(), five(), six(), seven(), eight(), nine())) {
                    view.check(matches(isEnabled()));
                }
                break;
            case AFTER_20:
                for (ViewInteraction view : CollectionsKt.arrayListOf(zero(), one(), two(), three(), four(), five())) {
                    view.check(matches(isEnabled()));
                }
