...
import com.theglendales.alarm.jjmvvm.permissionAndDownload.DownloadableItem
...
//                                Log.d(TAG, "TESTING!!: $myTestPurchJSonObj ") //MYTEST!!
                        ...
                        itemIDsMap.forEach { (k,v) -> if(purchaseFound.indexOf(k) == -1) { // itemIDsMap 에서 "구매한목록(purchaseFound)" 에 없는 놈들은 다 false 로!
                            ...
                            if(myDownloaderInstance.checkDuplicatedFileOnDisk(fileNameAndFullPath)) { // 혹시나..구매한적도 없는데 만약 디스크에 있으면
                                ...
                                myDownloaderInstance.deleteFromDisk(downloadableItem)
                            }
    ...
    fun refreshItemIdsAndMp3UrlMap() { // 새로 받은 ringToneList 로 itemIDsMap 을 수정/업뎃해줌. initIAP() 에서 호출됨.
        ...
        for (i in receivedRingtoneClassList.indices) {
            itemIDsMap[receivedRingtoneClassList[i].id] = receivedRingtoneClassList[i].iapName //ex) itemIDsMap[1=trackID] = p1
    ...
    private fun refreshItemsPriceMap() {
        ...
        if(!itemIDsMap.isNullOrEmpty())
        {
            ...
            billingClient!!.querySkuDetailsAsync(myParams.build()) { myQueryResultYo, mySkuDetailsListYo -> // SAM .. object: SkuDetailsResponseListener 인터페이스 심기와 같음.
                ...
                rcvAdapterInstance!!.updateRecyclerView(receivedRingtoneClassList) // #$#$#$$#$#$!@#$!!#! FINALLY 여기서 rcView 를 업뎃-> onBindView 하게끔! #$#$@!$#@@$@#$#
            }
        } else {
...
//MAP UTIL functions. 내가 만든. <<<------------
    ...
    private fun downloadHandlerBridge(trId: Int, keepTheFile: Boolean,singlePurchase: Boolean) {
        ...
        if(singlePurchase) { // 단독 구매건. (sync 와 무관하고 app 에서 한개 샀을 때)
            ...
            myDownloaderInstance.multiDownloadOrNot(downloadableItem, keepTheFile) // 앱 최초 실행시 파일 정리.. 초기화.. (구매한 놈 있는지 확인.. 없어야될 놈 있으면 삭제 등..)
        }
