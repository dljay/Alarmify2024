...
class VuMeterHandler {
    ...
    fun LcVmIcController(playStatus: StatusMp) {
        Log.d(TAG, "LcVmIcController: playStatus= $playStatus")

        when (playStatus) {
            StatusMp.LOADING -> {
                    CoroutineScope(Dispatchers.IO).launch {
                        val isFinished = stopPrevAssignNew()
...
// ************************************ //
    ...
    private fun stopPrevAssignNew(): Boolean { // 코루틴 + Status 를 input parameter 로..
        try{
            ...
            Log.d(TAG, "stopPrevAssignNew: error message: $e")
            return false
        }
    }
