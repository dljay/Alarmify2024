...
class DiskSearcher(val context: Context)
{
    ...
    fun rtSearcher(): List<Uri>
    {
        ...
        // 만약 폴더가 없을때는 폴더를 생성
        if(!myDir.exists()) {
            Log.d(TAG, "searchFile: Folder $myDir doesn't exist. We'll create one")
            myDir.mkdir()
        }
        // 폴더는 있는데 파일이 없을때.. 그냥 return
        if(myDir.listFiles().isNullOrEmpty()) {
            Log.d(TAG, "delFilesWithHyphenAndMore: NO FILES INSIDE THE FOLDER!")
            return emptyUriList
        }
        if(myDir.listFiles() != null)
        {
            for(f in myDir.listFiles())
            {
                ...
                try { // 미디어 파일이 아니면(즉 Pxx.rta 가 아닌 파일은) setDataSource 하면 crash 남! 따라서 try/catch 로 확인함.
                    ...
                    Log.d(TAG, "delFilesWithHyphenAndMore: unable to run mmr.setDataSource for the file=${f.name}. WE'LL DELETE THIS PIECE OF SHIT!")
                    f.delete()
                    return emptyUriList
                }
                ...
                Log.d(TAG, "delFilesWithHyphenAndMore: fileName= ${f.name}, fileDuration=$fileDuration")
                if(fileDuration==null) {
                    Log.d(TAG, "delFilesWithHyphenAndMore: Possible Corrupted file. Filename=${f.name}")
                    f.delete()
                }

                //2) hyphen(-) 포함이거나/'p' 가 없거나!/사이즈가=0 이면 => 삭제
                if(f.name.contains('-')||!f.name.contains('p')||f.length()==0L) {
                    Log.d(TAG, "!!! delFilesWithHyphenAndMore: ${f.name}")
                    if(f.length()==0L) {
                        Log.d(TAG, "delFilesWithHyphenAndMore: filesize prob 0? Filesize=${f.length()}")
                    }
                    f.delete()
                ...
                rtUris.add(fileUri)
                Log.d(TAG, "rtSearcher: file.name=${f.name}, file.path= ${f.path}, uri=$fileUri")
            }
