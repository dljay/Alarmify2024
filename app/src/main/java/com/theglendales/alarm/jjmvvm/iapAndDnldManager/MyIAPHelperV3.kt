...
// ****************** <1> 최초 SecondFrag 로딩 후 (과거 정보) 복원 관련
    ...
    suspend fun iap_D1_A_addPurchaseBoolToList(): List<Purchase> {

        Log.d(TAG, "iap_D1_A_addPurchaseBoolToList: <D1> called")
        multiDNLDNeededList.clear()
        purchaseFalseRtList.clear()

        return suspendCoroutine { continuation ->
            billingClient!!.queryPurchasesAsync(BillingClient.SkuType.INAPP) { _, listPurchs ->
                Log.d(TAG, "iap_D1_A_addPurchaseBoolToList: <D1> called ")
                continuation.resume(listPurchs)
            }
        }
    }
    fun iap_D1_B_addPurchaseBoolToList(listPurchs: List<Purchase>) {

        if (listPurchs.size > 0) // 구매건이 한 개 이상.
        {
            Log.d(TAG, "iap_D1_B_addPurchaseBoolToList: <D1> 총 구매 갯수=listPurchs.size=${listPurchs.size}")
            ...
            for (purchase in listPurchs)
            {
                ...
                if (purchase.quantity != 1 || indexOfRtObj < 0) { // 갯수가 1개 초과 or rtListPlusIAPInfo 리스트에 우리가 찾는 rtObj 이 없는 경우
                    if (purchase.quantity != 1) {
                        Log.d(TAG,"iap_D1_B_addPurchaseBoolToList: <D1-B-1> [IAP] 심각한 문제. Quantity 가 1개가 아님! Quantity=${purchase.quantity}")
                        toastMessenger.showMyToast("More than one IAP sku IDs.", isShort = true)
                    }
                    if (indexOfRtObj < 0) {
                        Log.d(TAG,"iap_D1_B_addPurchaseBoolToList: <D1-B-1> List 에서 현재 purchase.sku(=${purchase.skus[0]}) 에 매칭하는 rtObject 를 찾을 수 없음.")
                    }
                ...
                Log.d(TAG,"iap_D1_B_addPurchaseBoolToList: trackId=$trackID, purchase.skus[0]=${purchase.skus[0]}, p.skus(list)=${purchase.skus}")

                //purchaseFound.add(trackID) //For items that are found(purchased), add them to purchaseFound
        // **** [D1-B-2] :********************************>>> 구매 확인된 건
                if (purchase.purchaseState == Purchase.PurchaseState.PURCHASED)
                {
                    Log.d(TAG,"iap_D1_B_addPurchaseBoolToList: <D1-B-2> ☺ PurchaseState is PURCHASED for trackID=$trackID, itemName=$iapName")
                    ...
                    if (!myDiskSearcher.isSameFileOnThePhone(fileSupposedToBeAt)) {
                        ...
                        Log.d(TAG, "iap_D1_B_addPurchaseBoolToList: <D1-B-2> [멀티] 복원 다운로드 필요한 리스트에 다음을 추가: $iapName ")
                    }
                }
            // **** D1-B-3: 구매한적이 있으나 뭔가 문제가 생겨서 PurchaseState.Purchased 가 아닐때 여기로 들어옴. 애당초 구입한적이 없는 물품은 여기 뜨지도 않음!
                else {
                    Log.d(TAG,"iap_D1_B_addPurchaseBoolToList: <D1-B-3> iapName=$iapName, trkID=$trackID, 구매 기록은 있으나 (for some reason) PurchaseState.Purchased(X)- Phone 에서 삭제 요청 ")
                    //myDiskSearcher.deleteFromDisk(fileSupposedToBeAt)
                }
            }//end of for loop
        // **** D1-B-4: purchaseBool=false 인 item 들 -> 삭제할 리스트에 추가해줌.
            Log.d(TAG, "iap_D1_B_addPurchaseBoolToList: <D1-B-4> [END of FOR LOOP]")
        }//end of if(listPurchs.size > 0)
        // **** D1-B-5: 애당초 구매건이 하나도 없는 경우
        else {
            Log.d(TAG, "iap_D1_B_addPurchaseBoolToList: <D1-B-5>: 구매건이 하나도 없음! ")
        }
        ...
        Log.d(TAG, "iap_D1_B_addPurchaseBoolToList: <D1-B-6> ************ iap_D1_B_addPurchaseBoolToList() 끝나는지점 *****")
    }

    //<D2> 현재 리스트에 상품별 가격 적어주기 (ex. $1,000)
    suspend fun iap_D2_A_addPriceToList(): List<SkuDetails> {
        Log.d(TAG, "iap_D2_A_addPriceToList: <D2_A> called")
        ...
        return suspendCoroutine { continuation ->
            billingClient!!.querySkuDetailsAsync(myParams.build()) {queryResult, skuDetailsList ->
                if(queryResult.responseCode == BillingClient.BillingResponseCode.OK && skuDetailsList!=null)
                {
                    ...
                    Log.d(TAG, "iap_D2_A_addPriceToList: <D2-A> Finished(X) - Error! XXX loading price for items")
    ...
    fun iap_D2_B_addPriceToList(skuDetailsList: List<SkuDetails>) {
        for(skuDetails in skuDetailsList)
        {
            ...
            Log.d(TAG,"iap_D2_B_addPriceToList : <D2-B> item title=${skuDetails.title} b)item price= ${skuDetails.price}," +
                    " c)item sku= ${skuDetails.sku}")
        }
        Log.d(TAG, "iap_D2_B_addPriceToList: <D2-B> Finished (O)")
        //todo: 다운로드 관련 여기서..?
    }
